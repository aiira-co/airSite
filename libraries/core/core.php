<?php
    ...
    class Core{
        ...
        public static function getModel($model, $path = null){
    			$node = CORE::getInstance('Node');
                $file = 'components'. DS .$node->aleph.DS.$model .'.php';
                ...
                if (file_exists($file)){
    				...
    				$file = 'components'. DS .$node->aleph.DS.'models'.DS.$model.'.php';
    				if (file_exists($file)){
    					$fileFound = true;
    			...
    			if($fileFound){
    				...
    				$model = ucfirst($modelExplode[0]).ucfirst($modelExplode[1]);

                    $modelObj = CORE::getInstance($model);

                    return $modelObj;
    			}else{
    				echo 'the model(file) '.$model.'.php does not exist';
    			}
